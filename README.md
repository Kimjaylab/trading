...
| 구간 | 전략 | 진입 핵심 로직 | 부분익절 / 전량익절 / 손절 |
|---|---|---|---|
| 0~10분 | 돌파매매 | 직전고점/시초가 실제 돌파 + 강한 양봉 확인 후 진입 | +2% / +3% / -3% |
| 10~30분 | 눌림목매매 | 첫 상승(시가대비 1.5%↑) 확인 후 거래량 감소하는 '건강한 눌림' + 5일선/기준선 근접 + 분봉 반등 확인 | +5% / +8% / -4% |
| 30분~마감 | 추세+눌림목 | 5일선/기준선 위 우상향 추세 유지 + 거래량 생존 + 눌림 후 양봉 | +7% / +10% / -4% |

손절/익절 % 및 부분익절 비중(기본 50%)은 `config/config.yaml`의 `strategies.*.partial_take_profit_pct`
/ `full_take_profit_pct` / `hard_stop_pct` / `partial_exit_fraction`에서 조정한다. `phases`에서
시간 경계값도 조정할 수 있다. 각 전략은 손절/체결강도 붕괴/추세훼손/장마감 임박 등 추가적인
질적 청산 조건도 함께 검사한다(전략별 소스 참고) - 위 표는 정량적 손익 기준만 요약한 것이다.
추세+눌림목 전략(원데이 단타)으로 매수한 종목은 예외 없이 장 마감 직전(`force_exit_before_close_minutes`)
전량 매도된다.

**부분 익절 메커니즘**: 부분익절 임계값에 처음 도달하면 설정된 비중(기본 50%)만 매도하고
나머지는 계속 보유하며, 이후 전량익절/손절/기타 청산 조건 중 먼저 도달하는 쪽으로 잔량을
정리한다. 부분 청산은 보유종목수·연속손실 카운트에 영향을 주지 않는다(전량 청산 시에만
카운트된다).

**실거래 브로커의 stop/target 값 처리**: `KISBroker`/`KISOverseasBroker`의 `get_positions()`는
매번 새로 API를 호출해 만든 `Position`을 반환하므로 손절가/목표가를 알 수 없다(항상 0.0).
`BacktestEngine._process_exits()`가 진입 시점에 내부에 저장해둔 실제 stop/target 값으로
이를 덮어쓴 뒤 전략의 청산 판단에 넘긴다 - 이 처리가 없으면 실거래에서 손절이 전혀
발동하지 않는 심각한 버그가 된다(개발 중 발견해 수정함).
