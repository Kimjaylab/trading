...
src/trading/
  data/           # MarketSnapshot, MarketDataProvider 인터페이스
                  ...
                  # + KISMarketDataProvider(국내 실시간 시세, 일부 피처 미구현)
  ...
  brokers/        # BrokerClient 인터페이스 + PaperBroker(시뮬레이션)
                  # + KISSession(공용 인증) + KISBroker(국내) + KISOverseasBroker(해외)
...
코드는 수정할 필요 없음). 실시간 국내 시세는 `data/kis_provider.py`의
`KISMarketDataProvider`가 KIS REST API로 현재가/호가/분봉/일봉을 조회한다.

### 브로커
`brokers/kis_broker.py`(국내주식)와 `brokers/kis_overseas_broker.py`(미국 등 해외주식)에
한국투자증권 Open API 어댑터를 마련해뒀다. 토큰 발급/헤더 구성은 `brokers/kis_session.py`의
`KISSession`으로 공유한다(브로커와 시세 공급자가 같은 세션을 공유해 토큰 재발급을 최소화).

**신뢰도가 다르다는 점을 알아야 한다**:
- 국내주식 주문(`KISBroker`)의 TR_ID(`TTTC0802U` 등)와 잔고 응답 필드(`pdno`, `hldg_qty`,
  `pchs_avg_pric`)는 비교적 널리 문서화된 값이라 확신도가 높은 편이다.
- 해외주식 주문(`KISOverseasBroker`)의 TR_ID(`JTTT1002U` 등)는 커뮤니티 래퍼에서 참고한
  값으로, 이 환경에서 KIS 공식 문서와 대조하거나 실제 호출로 검증하지 못했다.
  **확신도가 낮다** - 파일 상단 주석에 검증 우선순위를 적어뒀다.
- `KISMarketDataProvider`는 현재가/호가/분봉/일봉만 구현했고, 프로그램매매·외국인/기관
  수급·VI·섹터/테마 피처는 대응 엔드포인트를 확인하지 못해 중립값(0/False)으로 채운다.
  즉 실거래에서는 이 피처들의 실질적인 영향력이 사라진다 - ML 최적화가 자연히 가중치를
  낮추긴 하겠지만, 정확한 판단을 원하면 별도 데이터 소스 연동이 필요하다.
...
`--broker kis`는 현재 국내(KRX) 시세 공급자만 구현되어 있다. 미국주식은 주문
어댑터(`KISOverseasBroker`)만 준비되어 있고, 실시간 해외 시세 공급자는 아직 없다 -
미국장 페이퍼트레이딩은 `--broker paper`로 로직만 검증할 수 있다.
